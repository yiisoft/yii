...
(такое, как, например, `create_news_table`). Как будет показано далее,
...
class m101129_185401_create_news_table extends CDbMigration
{
    ...
    /*
	// если требуется выполнить изменения в транзакции, используйте safeUp/safeDown
	// вместо up/down
...
методы [CDbCommand]. И это на самом деле так. Единственно отличие в том, что
...
При применении сложных миграций для соблюдения целостности и связности
...
Yii при применении миграции начнёт транзакцию и, затем, выполнит код в
...
> `up()` и `down()`. В случае MySQL, некоторые выражения SQL могут вызвать
...
по очереди запустит метод `up()` в каждом классе миграции в порядке его создания.
...
миграций и новые, ещё не применённые миграции.
...
миграций. По умолчанию оно равно `tbl_migration`. Структура таблицы следующая:
