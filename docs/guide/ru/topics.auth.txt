...
[статье в Википедии](http://en.wikipedia.org/wiki/Role-based_access_control).
...
Роль по умолчанию автоматически назначается каждому пользователю, включая гостей.
...
К примеру, следующий код определяет две роли: `authenticated` и `guest`,
которые соответственно применяются к аутентифицированным пользователям и гостям.
