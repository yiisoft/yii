...
Active Record реалізує популярний підхід об'єктно-реляційного проектування (ORM). Кожен клас AR відображає таблицю (або представлення) бази даних, екземпляр AR - рядок в цій таблиці, а загальні операції CRUD реалізовані як методи AR. В результаті, ми можемо працювати із більшою об'єктно-орієнтованістю. Наприклад, використовуючи наступний код, можна вставити новий рядок у таблицю `tbl_post`.
...
Нижче ми покажемо, як налаштувати та використовувати AR для реалізації CRUD-операцій, а в наступному розділі - як використовувати AR для роботи зі зв'язаними таблицями. Для прикладів у цьому розділі ми будемо використовувати наступну таблицю. Зверніть увагу, що при використанні БД MySQL у SQL-виразу нижче `AUTOINCREMENT` слід замінити на `AUTO_INCREMENT`.
...
> Tip|Підказка: Оскільки класи AR часто з'являються у багатьох місцях коду, ми можемо замість включення класів по одному, додати цілу папку з AR-класами. Приміром, якщо AR-класи знаходяться у папці `protected/models`, ми можемо налаштувати додаток наступним чином:
> ~~~
> [php]
> return array(
> 	'import'=>array(
...
За замовчуванням ім'я AR-класу збігається із назвою таблиці в базі даних. Якщо вони розрізняються, буде потрібно перевизначити метод [tableName()|CActiveRecord::tableName]. Метод [model()|CActiveRecord::model] оголошується для кожного AR-класу.
...
> }
> ~~~
> Замість того, щоб повертати повне ім'я таблиці, ми повертаємо ім'я таблиці без префікса і містимо його у подвійні фігурні дужки.
...
як різні СУБД працюють з регістрозалежними іменами по-різному. Наприклад, PostgreSQL вважає ім'я стовпців регістронезалежними за замовчуванням, і ми повинні містити ім'я стовпця у лапки в умовах запиту, якщо ім'я стовпця має великі літери. Використання нижнього регістру допомагає уникнути даної проблеми.
...
*Вище ми викликаємо метод `find` через `Post::model()`. Запам'ятайте, що статичний метод `model()` обов'язковий для кожного AR-класу. Цей метод повертає екземпляр AR, що використовується для доступу до методів рівня класу (дещо схоже зі статичними методами класу) у контексті об'єкту.*

Якщо метод `find` знаходить рядок, відповідний умовам запиту, він повертає екземпляр класу `Post`, властивості якого містять значення відповідних полів рядка таблиці. Далі ми можемо читати завантажені значення аналогічно звичайним властивостями об'єктів, наприклад, `echo $post->title;`.

У випадку, якщо в базі немає даних, що відповідають умовам запиту, метод `find` поверне значення null.

Параметри `$condition` та `$params` використовуються для уточнення запиту. В даному випадку `$condition` може бути рядком, що відповідає оператору `WHERE` у SQL-виразі, а `$params` — масивом параметрів, значення яких повинні бути прив'язані до маркерів, зазначених у `$condition`. Наприклад:
...
> Note|Примітка: У наведеному вище прикладі, нам може знадобитися взяти у лапки звернення до стовпця `postID` для деяких СУБД. Наприклад, якщо ми використовуємо СУБД PostgreSQL, нам слід писати умову як `"postID"=:postID`, бо PostgreSQL за замовчуванням вважає ім'я стовпця регістронезалежним.
...
Іменовані групи найчастіше описуються у методі [CActiveRecord::scopes()] парами ім'я-умова. 
...
Іменовані групи умов зазвичай використовуються як модифікаторів для методу `find`. Можна використовувати декілька груп для отримання більш специфічного результату. Приміром, щоб знайти останні опубліковані записи можна використовувати наступний код:
...
Іменовані групи умов можуть бути параметризовані. Приміром, нам знадобилося задати кількість публікацій для групи `recently`. Щоб це зробити, замість того, щоб описувати групу в методі [CActiveRecord::scopes], нам необхідно описати новий метод з таким же ім'ям, як у групи умов:
