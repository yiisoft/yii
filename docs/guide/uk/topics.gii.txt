...
> Note|Примітка: Gii є інструментом розробника. Тому він повинен бути встановлений виключно на компʼютері або сервері розробника. Так як він може генерувати нові скрипти PHP, необхідно приділити особливу увагу безпеці (пароль, IP фільтри).
...
Якщо існуючий додаток використовує формат URL `path` (див. [красиві адреси URL](/doc/guide/topics.url)), ми можемо запустити Gii по URL `http://hostname/path/to/index.php/gii`. Може знадобитися додати наступні правила URL перед вже існуючими:
...
Gii можна розширювати двома способами: змінюючи існуючі шаблони кодогенераторов та створюючи свої генератори.
	...
	'modules'=>array(
		'gii'=>array(
			...
			'generatorPaths'=>array(
				'application.gii',   // псевдонім шляху
			),
		),
	),
);
...
Наведені вище налаштування змушують Gii шукати генератори в директорії із псевдонімом `application.gii` на додаток до стандартного `system.gii.generators`.
