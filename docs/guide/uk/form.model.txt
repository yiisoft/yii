...
отримання даних, що вводяться користувачем на сторінці авторизації. Оскільки
...
у формі авторизації позначка цієї опції буде знята.
...
class LoginForm extends CFormModel
{
	...
	public function authenticate($attribute,$params)
	{
		...
		if(!$this->_identity->authenticate())
			$this->addError('password','Невірне ім\'я користувача або пароль.');
	}
}
~~~

У наведеному вище коді, `username` та `password` — обов'язкові для заповнення поля,
...
параметр `on` — необов'язковий параметр, що встановлює список сценаріїв, де повинно
застосовуватися правило; а також інші параметри — пари ім'я-значення, які використовуються для
...
[php]
/**
 * @param string $attribute ім'я поля, яке будемо валідувати
...
Третій варіант — визначити псевдонім класу валідатора. У прикладі вище, ім'я
...
~~~
[php]
// ім'я користувача — обов'язкове поле форми
...
// у сценарії авторизації поле `password` повинно бути перевірене на відповідність вказаного імені користувача
...
Атрибут вважається безпечним, якщо він з'являється в правилі валідації, вживаному в даному сценарії. Наприклад,
...
Важливо пам'ятати, що правила валідації використовуються для перевірки введених користувачем даних, 
...
Після перевірки всі можливі помилки знаходяться в об'єкті моделі. Ми можемо отримати їх через 
