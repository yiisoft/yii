...
Перед прочитанням даного розділу рекомендується прочитати [документацію по Selenium](http://seleniumhq.org/docs/) і [документацію по PHPUnit](http://www.phpunit.de/wiki/Documentation). Далі ми підсумуємо основні принципи написання функціональних тестів у Yii:

 * Як і модульний тест, функціональний тест пишеться в класі `XyzTest`, успадковує клас [CWebTestCase], де `Xyz` — ім'я класу, що підлягає тестуванню. Ми можемо використовувати всі методи класу `PHPUnit_Extensions_SeleniumTestCase`, бо він є предком класу [CWebTestCase].

 * Клас функціонального тесту зберігається у файлі з ім'ям `XyzTest.php`. За угодою, файл функціонального тесту може зберігатися в директорії `protected/tests/functional`.

 * Основний вміст класу тесту — набір тестових методів з іменами `testAbc`, де `Abc` — часто ім'я тестованої особливості. Наприклад, для тестування особливості входу користувача у нас є метод `testLogin`.
...
Тепер ми опишемо, як протестувати функцію відображення запису [демо-блога](http://www.yiiframework.com/demos/blog). Спочатку ми пишемо тестовий клас, як показано нижче. Зазначимо, що тестовий клас успадковує від базового класу, який ми тільки що описали:
