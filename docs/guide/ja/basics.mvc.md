...
MVCはユーザインタフェースの考慮からビジネスロジックを分離することを目的としています。
...
MVCを実装する他に、Yiiは`アプリケーション`と呼ばれるフロントコントローラも導入しています。
...
![Yiiアプリケーションの典型的なワークフロー](flow.png)
...
要求されている[コントローラ](/doc/guide/basics.controller)と[アクション](/doc/guide/basics.controller#action)を決定します。
