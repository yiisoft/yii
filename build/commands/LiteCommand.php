...
/**
 * LiteCommand generates yeelite.php by merging commonly used Yee class
...
DESCRIPTION
  This command generates yeelite.php by merging commonly used Yee class
  files into a single one and removes all comments and trace statements.

  You should not execute this command unless you change some framework
  file and need to update yee.php accordingly.
...
";
		...
		$content=$this->minifyYee(dirname(__FILE__).'/lite/index.php');
		$content="<?php\n".preg_replace('/^(\?>|<\?php)/mu','',$content)."\n?>";
		$content=$this->stripComments($content);
		$content=preg_replace('/^require(_once)?.*\s*;\s*$/mu','',$content);
		$content=preg_replace('/^\s*Yee::trace.*\s*;\s*$/mu','',$content);
//		$content=preg_replace('/^\s*Yee::(begin|end)Profile.*\s*;\s*$/mu','',$content);
		$content=$this->stripEmptyLines($content);
		$content=substr_replace($content,$comments,5,0);
		file_put_contents(YII_PATH.DIRECTORY_SEPARATOR.'yeelite.php',$content);
		echo "Done.\n";
	}

	protected function minifyYee($entryScript)
	{
		try
		{
			ob_start();
	...
	protected function unfoldFile($fileName)
	{
		...
		$pattern='^(Yee::import|require_once|include_once)\s*\(.*?[\'"]([^\*]*?)[\'"].*?\);';
