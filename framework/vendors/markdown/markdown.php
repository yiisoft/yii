...
class Markdown extends MarkdownExtra_Parser
{
	...
	function getHighlightFencedCode($codeblock)
	{
	...
	function getHighlightTag($codeblock)
	{
	...
	function highlightCodeBlock($class, $code)
	{
		$hl = $this->getHighLighter($class);
		if($hl)
		{
			$class_len = strpos($code, $class)+strlen($class);
			return $hl->highlight(ltrim(substr($code, $class_len)));
		}
	}
	...
	function getHighLighter($options)
	{
		...
		{
			require_once(dirname(__FILE__).'/../TextHighlighter/Text/Highlighter.php');
			require_once(dirname(__FILE__).'/../TextHighlighter/Text/Highlighter/Renderer/Html.php');
		}
		$lang = current(preg_split('/\s+/', substr(substr($options,1), 0,-1),2));
		$highlighter=Text_Highlighter::factory($lang,$this->getHiglightConfig($options));
		$highlighter->setRenderer(new Text_Highlighter_Renderer_Html());
		return $highlighter;
	}

	function getHiglightConfig($options)
	{
		$config['use_language'] = true;
		$config['numbers'] = $this->hasInlineOption('show-numbers', $options);
		$config['numbers_start'] = $this->getInlineOptionValue('first-line', $options);
		if($config['numbers_start'] !== null)
			$config['numbers'] = true;
		$config['tabsize'] = $this->getInlineOptionValue('tab-size', $options);
		return $config;
	}

	function getInlineOptionValue($name, $str)
	{
		if(preg_match('/'.$name.'(\s*=\s*(\d+))?/i', $str, $v) && count($v) > 2)
			return $v[2];
	}

	function hasInlineOption($name, $str)
	{
		return stripos($str, $name) !== false;
	}
}
